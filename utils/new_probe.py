...
# Global dictionary to track spoofed faces
SPOOFED_FACES = {}
...
def antispoof_probe(pad, info, user_data):
    """Process antispoofing results - MARK spoofed faces but DON'T remove them"""
    try:
            ...
            try:
                ...
                while l_obj is not None:
                    try:
                        ...
                        # Process antispoofing results
                        real_prob, spoof_prob = get_antispoof_results(obj_meta)

                        if real_prob is not None and spoof_prob is not None:
                            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            print(f"[{timestamp}] {source_name}: Face ID {obj_meta.object_id} - Real: {real_prob:.3f}, Spoof: {spoof_prob:.3f}")

                            # Set display text
                            set_antispoof_display_text(obj_meta, real_prob, spoof_prob)

                            # Mark spoofed faces in global dictionary
                            if spoof_prob > 0.7:
                                print(f"[{timestamp}] {source_name}: SPOOF DETECTED! Face ID {obj_meta.object_id}")

                                # Add to global spoofed faces dictionary
                                face_key = f"{source_id}_{obj_meta.object_id}"
                                SPOOFED_FACES[face_key] = True

                                # Update label to indicate spoofed
                                if hasattr(obj_meta, 'obj_label'):
                                    base_label = obj_meta.obj_label.split(' (SPOOFED)')[0]
                                    obj_meta.obj_label = f"{base_label} (SPOOFED)"

                            # Update object label for real faces
                            elif real_prob > 0.6 and hasattr(obj_meta, 'obj_label'):
                                base_label = obj_meta.obj_label.split(' (Real)')[0].split(' (SPOOFED)')[0]
                                obj_meta.obj_label = f"{base_label} (Real)"

                                # Remove from spoofed list if it was previously marked
                                face_key = f"{source_id}_{obj_meta.object_id}"
                                if face_key in SPOOFED_FACES:
                                    del SPOOFED_FACES[face_key]
        ...
        print(f"CRITICAL ERROR in antispoof_probe: {str(e)}")

    return Gst.PadProbeReturn.OK

def set_antispoof_display_text(obj_meta, real_prob, spoof_prob):
    """Set antispoofing results in the object display text"""
    try:
        ...
        if hasattr(obj_meta, 'text_params'):
            obj_meta.text_params.display_text = f"{status} (R:{real_prob:.2f}, S:{spoof_prob:.2f})"
            obj_meta.text_params.font_params.font_color.set(*color)

    except Exception as e:
        print(f"ERROR setting antispoof display text: {str(e)}")


def get_antispoof_results(obj_meta):
    """Extract antispoofing probabilities from tensor metadata"""
    try:
        ...
        while l_user_meta:
            try:
                ...
                # Check if this is from antispoofing model (unique ID 3)
                if (user_meta.base_meta.meta_type == pyds.NvDsMetaType.NVDSINFER_TENSOR_OUTPUT_META):
                    ...
                    # Check if this is our antispoofing model by unique ID
                    if hasattr(tensor_meta, 'unique_id') and tensor_meta.unique_id == 3:
                        ...
                        # Extract probabilities (assuming [1, 3] output shape)
                        output_data = []
                        for i in range(3):  # 3 classes: background, real, spoof
                            try:
                                ...
                                print(f"ERROR getting antispoof detection {i}: {str(e)}")
                                break

                        if len(output_data) == 3:
                            # Convert to numpy array and apply softmax
                            output_array = np.array(output_data, dtype=np.float32)
                            print(f"Raw outputs: {output_array}")
                            max_val = np.max(output_array)
                            exp_output = np.exp(output_array - max_val)
                            output_array = exp_output / np.sum(exp_output)
                            print(f"After softmax: {output_array}")
                            if np.any(output_array < 0) or np.any(output_array > 1):
                                print(f"WARNING: Invalid probabilities: {output_array}")
                                output_array = np.clip(output_array, 0, 1)
                                output_array = output_array / np.sum(output_array)
                            return output_array[1], output_array[2]
            except Exception as e:
                print(f"CRITICAL ERROR in get_antispoof_results: {str(e)}")
                traceback.print_exc()
            #return None, None

                            # Apply softmax if needed
                            #if output_array.sum() > 1.1:  # Check if softmax needed
                            #    exp_output = np.exp(output_array - np.max(output_array))
                            #    output_array = exp_output / exp_output.sum()

                            # Return real and spoof probabilities
                            #return output_array[1], output_array[2]  # real, spoof

            #except Exception as e:
            #    print(f"ERROR processing antispoof user meta: {str(e)}")
            #    traceback.print_exc()
            #finally:
            ...
            try:
                ...
                while l_obj is not None:
                    try:
                        ...
                        face_key = f"{frame_meta.source_id}_{obj_meta.object_id}"
                        if face_key in SPOOFED_FACES:
                            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            print(f"[{timestamp}] Skipping spoofed face ID {obj_meta.object_id} (global tracking)")
                            continue
                        real_prob, spoof_prob = get_antispoof_results(obj_meta)
                        if spoof_prob is not None and spoof_prob > 0.5:
                            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            print(f"[{timestamp}] Skipping spoofed face ID {obj_meta.object_id} (spoof prob: {spoof_prob:.3f})")
                            SPOOFED_FACES[face_key] = True
                            continue
