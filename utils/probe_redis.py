    ...
    while True:
        ...
        try:
            ...
            if accepted:
                ...
                # 🔥 Publish to Redis Stream for dashboard
                try:
                    stream_id = redis_client.xadd(
                        "attendance_stream",
                        {
                            "company_id": str(task["company_id"]),
                            "emp_id": str(task["emp_id"]),
                            "first_name": task["first_name"],
                            "last_name": task["last_name"],
                            "check_type": direction,
                            "attendance_date": str(task["attendance_date"]),
                            "attendance_time": str(task["attendance_time"]),
                            "image_url": task["image_url"] or ""
                        }
                    )

                    logger.info(f"[REDIS STREAM PUBLISHED] ID={stream_id} emp={task['emp_id']}")
