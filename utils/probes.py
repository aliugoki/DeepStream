...
# ------------------------------------------------------------
# Face save state (SAFE, CLEAN, AUTO-CLEANUP)
# ------------------------------------------------------------

FACE_SAVE_STATE = {}
FACE_SAVE_LOCK = multiprocessing.Lock()

FACE_SAVE_RETENTION_DAYS = 2      # keep today + yesterday
FACE_SAVE_CLEAN_INTERVAL = 300    # seconds (5 min)
_LAST_FACE_CLEANUP_TS = 0


def cleanup_face_save_state():
    """Remove old face-save entries to avoid memory leak."""
    ...
    with FACE_SAVE_LOCK:
        keys_to_delete = [
            k for k in FACE_SAVE_STATE
            if k[2].toordinal() < cutoff_ord
        ]
        ...
        if keys_to_delete:
            logger.info(f"🧹 FACE_SAVE_STATE cleanup: removed {len(keys_to_delete)} entries")
...
def should_save_face(source_id, emp_id, tracker_id):
    """
    Allow exactly ONE face save per (camera, employee, day)
    """
    today = date.today()
                            ...
                            if is_in_detection_area(obj_meta):
                                ...
                                if int_best_match_id is not None:
                                    ...
                                    if first_name or last_name or emp_id_from_db or image_path or COMPANY_ID:
                                        ...
                                        if should_save_face(source_id, int_best_match_id, obj_meta.object_id):

                                                enc_ctx = get_obj_encoder(0)

                                                cam_name = camera_name.replace(" ", "_")
                                                save_dir = os.path.join("/workspace/data/attendance_faces", cam_name, current_date.strftime("%Y-%m-%d"))

                                                os.makedirs(save_dir, exist_ok=True)

                                                filename = f"{int_best_match_id}_{obj_meta.object_id}_{current_timestamp.strftime('%H%M%S')}.jpg"
                                                save_path = os.path.join(save_dir, filename)

                                                save_face_crop_probe(
                                                    enc_ctx,
                                                    gst_buffer,
                                                    frame_meta,
                                                    obj_meta,
                                                ...
                                                logger.info(f"✅ Saved face crop: {save_path}")
...
# ------------------------------------------------------------
# Object encoder context (GLOBAL, ONE TIME)
# ------------------------------------------------------------
_obj_enc_ctx = None
...
# ------------------------------------------------------------
# SAFE SGIE / NVOSD PROBE (NO FRAME MAPPING)
# ------------------------------------------------------------
...
def save_face_crop_probe(enc_ctx, gst_buffer, frame_meta, obj_meta, save_path):
    ...
    enc_args.isFrame = 0
    enc_args.quality = 90
    enc_args.fileNameImg = save_path   # ✅ ABSOLUTE PATH
...
# ------------------------------------------------------------
# Attach probe (NVOSD sink OR tiler src)
# ------------------------------------------------------------
