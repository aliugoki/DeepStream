...
def sgie_feature_extract_probe(pad, info, user_data):
    ...
    while l_frame:
        ...
        while l_obj:
            ...
            # Line crossing logic (attendance trigger only)
            ...
            # 🔥 NO FACE ALIGNMENT — DIRECT FEATURE EXTRACTION
            ...
            if feat is not None:
                ...
                logger.debug(
                    "Recognition src=%s uid=%s score=%.3f",
                    src, uid, score if score else -1
                )

                # ✅ Recognition ALWAYS happens
                if uid and score >= RECOGNITION_THRESHOLD:
                    fname, lname, _, img = get_user_info(uid, COMPANY_ID)
                    label = f"{fname} {lname}"

                    # ✅ Attendance ONLY when direction is detected
                    if direction and should_save_face(src, uid):
                        save_dir = f"/workspace/data/attendance_faces/Cam_{src}/{date.today()}"
                        os.makedirs(save_dir, exist_ok=True)

                        img_path = f"{save_dir}/{uid}_{now:%H%M%S}.jpg"

                        args = pyds.NvDsObjEncUsrArgs()
                        args.saveImg = 1
                        args.fileNameImg = img_path
                        args.quality = 90
