            ...
            with closing(conn.cursor(cursor_factory=psycopg2.extras.DictCursor)) as cursor:
                ...
                for record in records:
                    ...
                    payload = {
                            ...
                            "attendance_date": record['attendance_date'].isoformat(),  # <-- date to string
                            "attendance_time": record['attendance_time'].isoformat(),  # <-- time to string
                            "check_type": str(record['check_type']),
                            "camera_name": str(record['camera_name']),
                            "duration": str(record['duration']),
                            "check_in_id": str(record['check_in_id']),
                            "timestamp": f"{record['attendance_date'].isoformat()} {record['attendance_time'].isoformat()}",                            
...
def prepare_webhook_payload(attendance_record):
    ...
    return {
        ...
        # FIX: Convert date and time objects to ISO 8601 strings
        "attendance_date": attendance_record[5].isoformat(),
        "attendance_time": attendance_record[6].isoformat(),
        ...
        "timestamp": f"{attendance_record[5].isoformat()} {attendance_record[6].isoformat()}",
