...
def sgie_feature_extract_probe(pad, info, user_data):
    """Primary logic probe: Recognition, Line Crossing, and Attendance Logging."""
            ...
            if feat is not None:
                ...
                if uid and score >= RECOGNITION_THRESHOLD:
                    ...
                    if is_in_area(obj_meta) and should_save_face(src_id, uid, obj_meta.object_id):
                        ...
                        # Queue for DB worker
                        q.put({
                            "company_id": COMPANY_ID, "emp_id": uid, "first_name": fname, "last_name": lname,
                            "image_url": img_path, "attendance_date": now.strftime("%d-%m-%Y"),
                            "attendance_time": now.strftime("%H:%M:%S"), "check_type": direction or "auto",
...
def attendance_worker(q):
    """Background process for non-blocking database writes."""
