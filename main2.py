...
class MyRTSPMediaFactory(GstRtspServer.RTSPMediaFactory):
    ...
    def do_create_element(self, url):
        ...
        logger.info(f"RTSPMediaFactory: Creating new element for URL: {url.get_request_uri()}")
        ...
        if not rtsp_pipeline_bin:
            logger.error("Failed to parse RTSP media pipeline launch string.")
            return None

        appsrc_element = rtsp_pipeline_bin.get_by_name("mysource")
        if not appsrc_element:
            logger.error("Failed to find 'mysource' appsrc in RTSP media pipeline.")
            return None

        # Store a reference to the appsrc so we can push buffers to it.
        self.rtsp_appsrc = appsrc_element
        logger.info("RTSP Media Factory element created successfully.")
...
def appsink_new_sample_probe(sink, factory):
    ...
    if sample:
        ...
        if elapsed_ns >= 1_000_000_000: # Every 1 second
            ...
            logger.info(f"RTSP APPSINK (Encoded) FPS: {fps:.2f}")
            last_appsink_timestamp = current_time_ns
            frame_count_appsink = 0                       
        ...
        if factory and factory.rtsp_appsrc:
            ...
            if ret != Gst.FlowReturn.OK:
                logger.error(f"Failed to push buffer to RTSP appsrc: {ret.value_name}")
    ...
    try:
        ...
        if "video" in gstname.lower():
            if features.contains("memory:NVMM"):
                logger.info(f"Linking 'uridecodebin' pad with NVMM-capable caps: {caps.to_string()}")
                bin_ghost_pad = source_bin.get_static_pad("src")
                if not bin_ghost_pad:
                    logger.error("Failed to get source bin ghost pad")
                    return
                if not bin_ghost_pad.set_target(decoder_src_pad):
                    logger.error("Failed to link decoder src pad to source bin ghost pad")
            else:
                logger.error("Decodebin did not pick nvidia decoder plugin - no NVMM memory feature")
    except Exception as e:
        logger.error(f"Exception in cb_newpad: {str(e)}\n{traceback.format_exc()}")
...
def create_source_bin(index, uri):
    ...
    try:
        ...
        logger.error(f"Exception in create_source_bin: {str(e)}\n{traceback.format_exc()}")
        return None
...
def main(cfg):
    ...
    logger.info("DeepStream Face Recognition and RTSP Streaming Pipeline")
    ...
    try:
        ...
        logger.info(f"Loaded {len(known_face_features)} known face features")
    except Exception as e:
    ...
    # NEW: Converter for SGIE output to RGBA for Python probe
    sgie_output_converter = Gst.ElementFactory.make("nvvideoconvert", "sgie-output-converter")
    sgie_output_capsfilter = Gst.ElementFactory.make("capsfilter", "sgie-output-capsfilter")
    ...
    elements = [pipeline, streammux, pgie, tracker, sgie, sgie_output_converter, sgie_output_capsfilter, tiler, nvvidconv, nvosd, tee]
    if any(e is None for e in elements):
        logger.error("Failed to create one or more essential GStreamer elements.")
        return
    ...
    for i, (k, v) in enumerate(sources.items()):
        ...
        if source_bin:
            ...
            if srcpad.link(sinkpad) != Gst.PadLinkReturn.OK:
                logger.error(f"Failed to link source bin {i} to streammux.")
    ...
    # Configure the new sgie_output_converter for RGBA output
    sgie_output_caps = Gst.Caps.from_string("video/x-raw(memory:NVMM), format=RGBA")
    sgie_output_capsfilter.set_property("caps", sgie_output_caps)


    # --- Add and Link Main Pipeline Elements ---
    # MODIFIED LINKING: Insert sgie_output_converter and sgie_output_capsfilter
    main_elements = [pgie, tracker, sgie, sgie_output_converter, sgie_output_capsfilter, tiler, nvvidconv, nvosd, tee]
    for el in main_elements:
        pipeline.add(el)

    logger.info("Linking main pipeline elements: streammux -> pgie -> tracker -> sgie -> sgie_output_converter -> sgie_output_capsfilter -> tiler -> nvvidconv -> nvosd -> tee")
    if not streammux.link(pgie): logger.error("Failed to link streammux to pgie"); return
    if not pgie.link(tracker): logger.error("Failed to link pgie to tracker"); return
    if not tracker.link(sgie): logger.error("Failed to link tracker to sgie"); return
    
    # Link SGIE to the new converter
    if not sgie.link(sgie_output_converter): logger.error("Failed to link sgie to sgie_output_converter"); return
    if not sgie_output_converter.link(sgie_output_capsfilter): logger.error("Failed to link sgie_output_converter to sgie_output_capsfilter"); return

    # Link the converter's capsfilter to tiler
    if not sgie_output_capsfilter.link(tiler): logger.error("Failed to link sgie_output_capsfilter to tiler"); return
    
    if not tiler.link(nvvidconv): logger.error("Failed to link tiler to nvvidconv"); return
    if not nvvidconv.link(nvosd): logger.error("Failed to link nvvidconv to nvosd"); return
    if not nvosd.link(tee): logger.error("Failed to link nvosd to tee"); return

    # --- Create Output Branches from Tee ---
    # Branch 1: Display or Fake Sink
    logger.info("Creating display/fakesink branch...")
    display_queue = Gst.ElementFactory.make("queue", "display_queue")
    pipeline.add(display_queue)
    if not tee.link(display_queue): logger.error("Failed to link tee to display_queue"); return

    if cfg['pipeline']['display']:
        logger.info("Creating display sink (nveglglessink)...")
    ...
    if not display_sink: logger.error("Failed to create display/fake sink"); return
    display_sink.set_property("sync", False)
    pipeline.add(display_sink)
    if not display_queue.link(display_sink): logger.error("Failed to link display_queue to sink"); return

    # Branch 2: RTSP Streaming
    if cfg['pipeline']['enable_rtsp_streaming']:
        logger.info("Setting up RTSP streaming branch...")
        ...
        if any(e is None for e in rtsp_elements): logger.error("Failed to create RTSP branch elements"); return
        ...
        logger.info("Linking RTSP branch: tee -> queue -> ... -> appsink")
        if not tee.link(rtsp_queue): logger.error("Failed to link tee to rtsp_queue"); return
        if not rtsp_queue.link(rtsp_conv): logger.error("Failed to link rtsp_queue to rtsp_conv"); return
        if not rtsp_conv.link(rtsp_caps): logger.error("Failed to link rtsp_conv to rtsp_caps"); return
        if not rtsp_caps.link(rtsp_encoder): logger.error("Failed to link rtsp_caps to rtsp_encoder"); return
        if not rtsp_encoder.link(rtsp_appsink): logger.error("Failed to link rtsp_encoder to rtsp_appsink"); return
        ...
        logger.info(f"RTSP stream available at rtsp://<host-ip>:{port}{cfg['rtsp_server'].get('mount-point', '/ds-stream')}")


    # --- Attach Probes for Custom Logic ---
    logger.info("Attaching buffer probes for custom logic...")
    pgie_src_pad = pgie.get_static_pad("src")
    if pgie_src_pad:
        pgie_src_pad.add_probe(Gst.PadProbeType.BUFFER, pgie_src_filter_probe, 0)
    else: logger.error("Could not get pgie src pad for probe")

    # MODIFIED PROBE ATTACHMENT: Attach to sgie_output_capsfilter's src pad
    sgie_src_pad = sgie_output_capsfilter.get_static_pad("src") # <--- MODIFIED LINE
    if sgie_src_pad:
    ...
    else: logger.error("Could not get sgie_output_capsfilter src pad for probe") # <--- MODIFIED LOG MESSAGE
