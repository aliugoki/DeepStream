...
#def get_sources_from_form():
#    """Get RTSP sources from a form input"""
#    print("Enter RTSP sources (one per line). Press Enter on an empty line to finish:")
#
#    sources = {}
#    index = 0
...
#            break
#
#        name = input(f"Enter display name for source {index}: ").strip()
#        if not name:
#            name = f"Camera {index}"
#
#        sources[f"source_{index}"] = {
...
#    """Get basic pipeline configuration from user"""
#    config = {
#        'pipeline': {
#            'known_face_dir': input("Enter path to known faces directory: ").strip(),
#            'save_feature': input("Save detected features? (y/n): ").strip().lower() == 'y',
#            'display': input("Enable display output? (y/n): ").strip().lower() == 'y',
#            'is_aarch64': False,  # You can detect this automatically if needed
#            'enable_rtsp_streaming': False  # Can be enabled if needed
#        },
#        'tracker': {
#            'config-file-path': 'config/tracker_config.txt'  # Default path
#        }
#    }
#
#    if config['pipeline']['save_feature']:
#        config['pipeline']['save_feature_path'] = input("Enter path to save detected features: ").strip()
#
#    return config
...
def main():
    try:
        ...
        cfg = parse_args(cfg_path="config/my_pipeline.toml")
        ...
        log_info("Creating Sgie for antispoofing...")
        sgie_antispoof = Gst.ElementFactory.make("nvinfer", "secondary-antispoof-inference")
        if not sgie_antispoof:
            log_error("Unable to create antispoof sgie")
            return
        ...
        #pgie_person.set_property('config-file-path', 'config/new.txt')
        ...
        log_info("Setting antispoof sgie properties...")
        sgie_antispoof.set_property('config-file-path', 'config/spoof.txt')
        ...
        elements_to_add = [pgie_face, sgie,sgie_antispoof, tracker, tiler, nvvidconv, nvosd, caps, sink]
        for element in elements_to_add:
            pipeline.add(element)
            log_info(f"Added {element.name} to pipeline")

        log_info("Linking elements in the Pipeline...")
        elements = [streammux] + queues + [pgie_face, tracker, sgie,sgie_antispoof, tiler, nvvidconv, nvosd, sink]
        ...
        try:
            ...
            sgie_antispoof_src_pad = sgie_antispoof.get_static_pad("src")
            if sgie_antispoof_src_pad:
                log_info("Adding probe to antispoof sgie src pad")
                data = {
                    'known_face_features': known_face_features,
                    'save_feature': save_feature,
                    'save_path': save_path,
                ...
                sgie_antispoof_src_pad.add_probe(Gst.PadProbeType.BUFFER, antispoof_probe, data)
            else:
                log_error("Could not get antispoof sgie src pad")
